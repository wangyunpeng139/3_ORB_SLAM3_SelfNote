...
public:
    ...
    //设置参与了哪一图像帧的局部地图更新，在函数UpdateLocalKeyFrames中被更新
    long unsigned int mnTrackReferenceForFrame; 
    //只在函数 SearchInNeighbors 内部更新数值，标记在局部建图线程中,和哪个关键帧进行融合的操作，其记录的值就是帧的id
    long unsigned int mnFuseTargetForKF;     

    // Variables used by the local mapping
    long unsigned int mnBALocalForKF;   //标记参与了以哪一个KF为主导的局部BA优化，记录下该KF的id
    long unsigned int mnBAFixedForKF;  //标记自身作为固定帧参与了哪一个KF的局部BA优化，在BA结束后会被清除（=0）
    ...
    int mnMergeWords;
    float mMergeScore;
    /// 标记了当前关键帧是id为mnPlaceRecognitionQuery的位置重识别的候选关键帧
    long unsigned int mnPlaceRecognitionQuery;
    int mnPlaceRecognitionWords;
    float mPlaceRecognitionScore;

    bool mbCurrentPlaceRecognition;  //标记现在正在进行位置识别
    ...
    //!  Variables used by merging    （估计是怕merge地图前后位姿搞混乱了）
    cv::Mat mTcwMerge;
    cv::Mat mTcwBefMerge;
    cv::Mat mTwcBefMerge;
    cv::Mat mVwbMerge;
    cv::Mat mVwbBefMerge;
    IMU::Bias mBiasMerge;
    long unsigned int mnMergeCorrectedForKF;   //当前帧处于局部窗口中的时候，在这里会记录局部窗口的引领者（也就是局部窗口中的那个核心关键帧）
    ...
    KeyFrame* mPrevKF;    //关键帧的前一个关键帧，也就是LastKF
    KeyFrame* mNextKF;
...
protected:
    ...
    cv::Mat Tcw;  //世界坐标系  -->> 相机坐标系  ： 当前相机位姿
    cv::Mat Twc;  //当前相机位姿的逆
    cv::Mat Ow;    //相机光心(左目)在世界坐标系下的坐标,这里和普通帧中的定义是一样的
    cv::Mat Cw; // Stereo middel point. Only for visualization

    // IMU position
    cv::Mat Owb;   //世界坐标系下传感器IMU的位移

    // Velocity (Only used for inertial SLAM)
    cv::Mat Vw;  //这一部分的赋值，好像是在 IMu的初始化部分进行赋值，IMU的初始化是在局部线程里面的
    ...
    // Bad flags
    bool mbNotErase;         //KF允许擦除开关
    bool mbToBeErased;   //KF将要擦除标志位，必须在上面的开关打开情况下可以进行
    bool mbBad;    
...
public:
    GeometricCamera* mpCamera, *mpCamera2;          //这里面的1 和 2 都是什么？？
