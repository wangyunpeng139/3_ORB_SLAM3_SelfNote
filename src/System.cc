...
System::System(
    const string &strVocFile,              //ORBvoc.txt 
    const string &strSettingsFile,    //EuRoC.yaml 等配置文件
    const eSensor sensor,                   //指定使用的传感器
    const bool bUseViewer,              //是否可视化
    const int initFr,                               //默认为0
...
/*======================================================================
//功能： 输入为单目图像时的追踪器接口  (将读取的图像im传给Tracking线程)
//参数：
//例子：  跟踪单目相机帧数据        im 图片            timestamp 图片对应的时间戳
//注意： 这个函数就做了两件事
...
//! 改动：拓展为了 monocular_inertial
========================================================================*/
/**
 * im  :本次跟踪的对象
 * timestamp ： 当前帧图像的时间戳
 * vImuMeas :     存储两幅图像之间的imu数据（加速度+陀螺仪+时间戳）
*/
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    //! 1  检查是否需要切换运行模式
    {
        ...
        */
        ...
        if(mbActivateLocalizationMode)  //定位模式，只定位不建图（虽然还会提取特征点，但是不再创建关键帧）
        {
            ...
            //mpTracker：跟踪线程句柄～～
            //mbOnlyTracking = true，这里标记属于纯粹定位状态
            mpTracker->InformOnlyTracking(true); 
            mbActivateLocalizationMode = false;
        }
        ...
        {
            mpTracker->InformOnlyTracking(false);
            mpLocalMapper->Release();   //可以继续建图和产生关键帧了
            mbDeactivateLocalizationMode = false;
        }
    }
    ...
    //! 2  检查是否需要复位操作
    {
        ...
        else if(mbResetActiveMap)  //! v3增加部分，初始化为 false
        {
    ...
    //!  3 若有imu，向跟踪线程传递两幅图像之间的imu数据
    if (mSensor == System::IMU_MONOCULAR)
        // vImuMeas : 存储两幅图像之间的imu数据
        for(size_t i_imu = 0; i_imu < vImuMeas.size(); i_imu++)
            mpTracker->GrabImuData(vImuMeas[i_imu]);

    //!  4 向Tracking线程传递图片和及其时间戳
    //获取相机位姿的估计结果
...
{
    if(mpAtlas->GetCurrentMap()->KeyFramesInMap() < 12)  //! 后面加2个0
    {
        mpTracker->ResetActiveMap();
    }
    else
