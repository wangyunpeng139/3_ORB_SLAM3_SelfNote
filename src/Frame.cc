...
//单目
Frame::Frame(
    const cv::Mat &imGray,              //灰度图像
    const double &timeStamp,     //时间戳
     ORBextractor* extractor,       //特征点提取器
     ORBVocabulary* voc, 
     GeometricCamera* pCamera,      //相机模型
     cv::Mat &distCoef,                             //去畸变矩阵
     const float &bf,                                   //双目的时候才会在跟踪初始化的时候进行计算
     const float &thDepth, 
     Frame* pPrevF,                      //上一帧
     const IMU::Calib &ImuCalib
     )
     ...
     mpCamera2(nullptr), mTimeStereoMatch(0), mTimeORB_Ext(0)
{
    ...
    mfScaleFactor = mpORBextractorLeft->GetScaleFactor();   //比例系数
    ...
    mvLevelSigma2 = mpORBextractorLeft->GetScaleSigmaSquares();//获取sigma^2这个不知道有什么用的变量，  sigma^2：每层缩放系数的平方
    mvInvLevelSigma2 = mpORBextractorLeft->GetInverseScaleSigmaSquares();//计算sigma^2的倒数

    // ORB extraction
#ifdef SAVE_TIMES
    std::chrono::steady_clock::time_point time_StartExtORB = std::chrono::steady_clock::now();
#endif
    //对这个单目图像进行提取特征点操作  ,0 = 左目(单目)  1 = 右目
    //最后两个参数 与加速立体鱼眼相机有关
    //! wyp
    //!  1 提取ORB特征点
    ...
    mnCloseMPs = 0;  //v3 增加部分

	//初始化存储地图点句柄的vector
    //  static_cast  无检查的强制类型转换
    mvpMapPoints = vector<MapPoint*>(N,static_cast<MapPoint*>(NULL));
    ...
    if(pPrevF)    //上一帧图像
    {
        //!  4 如果上一帧有IMU线速度的话，直接先拿来作为本帧的线速度
        if(!pPrevF->mVw.empty())  
            mVw = pPrevF->mVw.clone();
    }
    else
...
{
    //?  vLapping是做什么的？？   在初始化的时候  {x0,x1} = {0,1000}，与加速立体鱼眼相机有关
    vector<int> vLapping = {x0,x1};
        /**这里的提取器句柄就是一个函数指针...或者说,是运算符更加合适
         * ORBextractor::operator()  
         * 参考链接：[http://blog.sina.com.cn/s/blog_5d2a81780100r9q0.html]
         * 注意：下面的运算符是属于ORBextractor::operator()的重载， (*mpORBextractorLeft)只是提示重载时属于指针对象mpORBextractorLeft的
         *      并不是说，调用的指针函数或者是什么的
         * */
    if(flag==0)
...
//根据mTcw更新关联的R和t
//更新 R和t，一般发生在imu对位姿mTcw进行预测后。这个时候还没有修改关联的R和t变量，所以在这里调用进行更新关联的数值
...
//TODO 这个函数未看，但是还算比较重要  2020-10-16
/**
