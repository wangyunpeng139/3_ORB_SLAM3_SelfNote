...
//BOOST_CLASS_EXPORT_GUID(Pinhole, "Pinhole")
     ...
     * v = fy * Y/Z + cy
    */
    // 相机坐标-->相机归一化平面坐标（xc*invzc，yc*invzc）；3.相机归一化平面坐标-->像素坐标(u,v)；
    //上面的xc*invzc和yc*invzc就是归一化平面的x,y坐标；计算所得u,v为像素坐标（像素坐标的原点在左上角，不再是图像中心）
    // 投影到当前帧中，之所以能够投影到当前帧就是因为：运动模型跟踪近似求出当前帧位姿
    //mvParameters[0] = fx
    //mvParameters[1] = fy
    //mvParameters[2] = cx
    //mvParameters[3] = cy
