...
Sim3Solver::Sim3Solver(
                KeyFrame *pKF1,                 //当前帧
                KeyFrame *pKF2,                 //当前帧的闭环候选帧【一级KF】（该KF的共视KF就是下面的【二级KF】）
                const vector<MapPoint *> &vpMatched12,     //vpCovKFi  【二级KF】与当前帧匹配的地图点
                const bool bFixScale,       //是否固定尺度（单目=不固定尺度）  // true，则是6DoFf优化（双目 RGBD）; false，则是7DoF优化（单目）
                vector<KeyFrame*> vpKeyFrameMatchedMP    //记录匹配地图点对应的vpCovKFi  【二级KF】
                ):
    ...
    pCamera1(pKF1->mpCamera), pCamera2(pKF2->mpCamera)
{
    ...
    //mN1 =  【二级KF】与当前帧匹配的地图点
    //虽然这么说，但是实际上大小就等于当前帧的地图点个数
    for(int i1=0; i1<mN1; i1++)
    {
        if(vpMatched12[i1])  //非空，表示当前帧上该点确实有被匹配上
        {
            // pMP1和pMP2是匹配的MapPoint
            MapPoint* pMP1 = vpKeyFrameMP1[i1];   //当前帧
            MapPoint* pMP2 = vpMatched12[i1];          //闭环候选帧
