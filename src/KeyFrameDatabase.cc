...
//! [PR] step 1 : DBoW2 candidate keyframes   检索三个最相似的关键帧（分别让如闭环候选帧和匹配候选帧）
/**
 *  
 * [in]  [out]  [out]  [in]=3
 * 
 * 与ORB_SLAM2中函数 DetectLoopCandidates 几乎很像
*/

void KeyFrameDatabase::DetectNBestCandidates(
    KeyFrame *pKF,                                              //[in] 检索的对象
    vector<KeyFrame*> &vpLoopCand,      //[out] nNumCandidates  个  闭环候选帧（不包括共视图里面的关键帧？？）
    vector<KeyFrame*> &vpMergeCand,    //[out]nNumCandidates  个  匹配候选帧    （不包括共视图里面的关键帧？？）
    int nNumCandidates
    )
{
    list<KeyFrame*> lKFsSharingWords;
    ...
    //!  Search all keyframes that share a word with current frame
    //!  找出和当前帧具有公共单词的所有关键帧（不包括与当前帧链接的关键帧）,查到的共享词向量的关键帧存放在lKFsSharingWords当中
    {
        unique_lock<mutex> lock(mMutex);

        // 获取与该关键帧连接的关键帧集合（不包括自己～）
        spConnectedKF = pKF->GetConnectedKeyFrames();
        // words是检测图像是否匹配的枢纽，遍历该pKF的每一个word
        for(DBoW2::BowVector::const_iterator vit=pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit != vend; vit++)
        {
            //!   提取所有包含该word的KeyFrame
            //TODO  mvInvertedFile 是关键帧数据库，所有插入的关键帧都在这里面，从而自然做到了遍历所有的关键帧
            list<KeyFrame*> &lKFs =   mvInvertedFile[vit->first];
            //!  然后对这些关键帧展开遍历
            for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++)
            {
                ...
                //  mnPlaceRecognitionQuery 标记了当前关键帧是id为mnPlaceRecognitionQuery的位置重识别的候选关键帧
                if(pKFi->mnPlaceRecognitionQuery!=pKF->mnId)   // pKFi还没有标记为pKF的候选帧
                {
                    pKFi->mnPlaceRecognitionWords=0;    //避免之前某次遍历数据的影响～
                    if(!spConnectedKF.count(pKFi))   // 与pKF局部链接的关键帧不进入闭环候选帧（就是闭环检测的时候要避免与与相近点匹配呗？？）
                    {

                        pKFi->mnPlaceRecognitionQuery=pKF->mnId;
                        lKFsSharingWords.push_back(pKFi);   // pKFi标记为pKF的候选帧，之后再次循环的时候就会直接跳过判断
                    }
                }
    ...
    // Only compare against those keyframes that share enough words
    int maxCommonWords=0;
    //! 遍历lKFsSharingWords，统计候选帧中与pKF具有共同单词最多单词数个数  ：  maxCommonWords
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        if((*lit)->mnPlaceRecognitionWords>maxCommonWords)
            maxCommonWords=(*lit)->mnPlaceRecognitionWords;
    ...
    // Compute similarity score.
    //!  再次遍历lKFsSharingWords，得到和当前关键帧共享单词数大于  minCommonWords   = 0.8×maxCommonWords，
    //!     存入   lScoreAndMatch   当中
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        KeyFrame* pKFi = *lit;
    ...
    // Lets now accumulate score by covisibility
    //!  遍历   lScoreAndMatch   当中的关键帧,  lScoreAndMatch  中每一个KeyFrame都把与自己共视程度较高的帧归为一组，
    ...
    for(list<pair<float,KeyFrame*> >::iterator it=lScoreAndMatch.begin(), itend=lScoreAndMatch.end(); it!=itend; it++)
    {
        ...
        for(vector<KeyFrame*>::iterator vit=vpNeighs.begin(), vend=vpNeighs.end(); vit!=vend; vit++)
        {
            ...
            accScore+=pKF2->mPlaceRecognitionScore;
            if(pKF2->mPlaceRecognitionScore>bestScore) // 统计得到组里分数最高的KeyFrame
            {
                pBestKF=pKF2;
                bestScore = pKF2->mPlaceRecognitionScore;
            }
    ...
    lAccScoreAndMatch.sort(compFirst);  //最高的排在前面

    vpLoopCand.reserve(nNumCandidates);   // nNumCandidates = 3 
    ...
    //! 遍历lAccScoreAndMatch，取出最靠前的几个,将该关键帧存入闭环候选帧spAlreadyAddedKF当中
    //spAlreadyAddedKF 没再使用了，这也是orb_SLAM3开发中的原因
