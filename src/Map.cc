...
// 设置参考地图点，在跟踪线程中的局部地图跟踪里面被调用
//需要注意的是这里的 vpMPs 是上一帧局部地图遗留下来的信息，这个时候本帧的局部地图还没有更新进来的（在跟踪线程下一步就会更新局部地图了）
...
//设定系统为vio模式，在system初始化的时候通过atlas里面调用该函数
